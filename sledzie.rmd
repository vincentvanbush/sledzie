...
Wczytanie bibliotek `dplyr` (przetwarzanie danych), `ggplot2` i `plotly` (prezentacja graficzna), `easyGgplot2` (rozszerzenia do `ggplot2`) oraz `caret` i ustawienie ziarna generatora liczb losowych.
...
```{r learning}
fit <- train(length ~ ., data = training, method = "rf", trControl = ctrl, verbose = TRUE)
```