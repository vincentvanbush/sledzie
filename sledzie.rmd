...
Poniższy wykres przedstawia zależność długości ryby (mierzonej z dokładnością ) od czasu (numeru sekwencyjnego próbki w zbiorze). Dodatkowo, próbki zosta
```{r length_diagram, out.width="100%", fig.height=5}
ggplotly(
  ggplot(processed.data[seq(1, nrow(processed.data), 50), ], aes(X, length)) +
    geom_point(aes(color=sst)) +
